...
{
	if(huart==&CMD_LINKER) // Motor Board receive data (filter)
	{
		switch(state)
		{
		case STATE_PREAMBLE1:  //#0
			if(inputBuf[0]=='M')  //hex :4D - "M" -fixed
				state=STATE_PREAMBLE2; //=1
			break;
		case STATE_PREAMBLE2: //#1
		...
		case STATE_ADDR://#2
			if(inputBuf[0]==BOARD_ADDR) //hex :43 --'C'--control board
			{
				...
				state=STATE_PREAMBLE1;
			break;
		case STATE_CMD: //#3  if 'R' = hex:0x52
...
/****************************************************************************************************
	...
	*Function: Control Motor Board command -'STOP'
	*Input Ref:  NO
	*Return Ref:NO
	*
****************************************************************************************************/
...
/****************************************************************************************************
	...
	*Function: UART to motor communication protocol TX
